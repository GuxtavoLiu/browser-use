		...
		try:
			...
			if browser_state_summary and model_output:
				...
				executed_event = CreateAgentStepEvent(
					...
					agent_task_id=str(self.task_id),  # Convert UUID to string
	...
	async def run(
		...
		try:
			...
			session_event = CreateAgentSessionEvent(
				id=str(self.session_id),  # Convert UUID to string
				user_id='',  # To be filled by cloud handler
			...
			task_event = CreateAgentTaskEvent(
				id=str(self.task_id),  # Convert UUID to string
				user_id='',  # To be filled by cloud handler
				agent_session_id=str(self.session_id),  # Convert UUID to string
				task=self.task,
				...
				paused=False,
				done_output=None,
				started_at=datetime.fromtimestamp(self._task_start_time),
				finished_at=None,
			)
