...
class Agent(Generic[Context]):
	...
	async def step(self, step_info: AgentStepInfo | None = None) -> None:
		...
		try:
			...
			if browser_state_summary and model_output:
				...
				executed_event = CreateAgentStepEvent(
					# id field will be auto-generated by backend model
					user_id='',  # To be filled by cloud handler
					agent_task_id=self.task_id,  # UUID 
					...
					# screenshot_url will be handled by the backend client
				)
	...
	async def run(
		...
		try:
			...
			# Emit CreateAgentSessionEvent at the end with all collected data
			if hasattr(self, '_session_start_time'):
				session_duration = time.time() - self._session_start_time
				# Get final URLs visited
				final_urls = self.state.history.urls() if self.state.history else []

				session_event = CreateAgentSessionEvent(
					id=self.session_id,  # UUID
					user_id='',  # To be filled by cloud handler
					browser_session_id=self.browser_session.session_id
					if hasattr(self.browser_session, 'session_id')
					else str(self.session_id),
					browser_session_live_url='',  # To be filled by cloud handler
					browser_session_cdp_url='',  # To be filled by cloud handler
					browser_state={
						'viewport': self.browser_profile.viewport if self.browser_profile else {'width': 1280, 'height': 720},
						...
						'initial_url': final_urls[0] if final_urls else None,
						'final_url': final_urls[-1] if final_urls else None,
						'total_pages_visited': len(final_urls),
				...
				self.event_bus.emit(session_event)

			# Emit CreateAgentTaskEvent at the end with all collected data
			if hasattr(self, '_task_start_time'):
				task_duration = time.time() - self._task_start_time
				done_output = self.state.history.get_last_done_output() if self.state.history else None
				
				task_event = CreateAgentTaskEvent(
					id=self.task_id,  # UUID
					user_id='',  # To be filled by cloud handler
					agent_session_id=self.session_id,  # UUID
					task=self.task,
					llm_model=self.model_name,
					agent_state=self.state.model_dump() if hasattr(self.state, 'model_dump') else {},
					# Additional fields populated at end
					...
					started_at=datetime.fromtimestamp(self._task_start_time) if hasattr(self, '_task_start_time') else None,
					finished_at=datetime.utcnow(),
				)
				self.event_bus.emit(task_event)
			...
			if self.settings.generate_gif:
				...
				if gif_path.exists():
					...
					gif_event = CreateAgentOutputFileEvent(
